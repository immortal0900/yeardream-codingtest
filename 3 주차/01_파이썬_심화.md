...
numbers = "   1  2  3   "   
print(numbers.split()) # ['1', '2', '3'] # 괄호안에 아무것도 안넣으면 연속된 공백들을 하나로 취급함
...
파이썬에서 문자열을 특정 구분자로 나눌 때, 다음 규칙이 적용됩니다:
- 구분자가 문자열의 시작에 있으면, 그 구분자 앞에는 "아무것도 없는" 빈 문자열이 있다고 간주합니다.
- 구분자가 문자열의 끝에 있으면, 그 구분자 뒤에는 "아무것도 없는" 빈 문자열이 있다고 간주합니다.
- 구분자가 연속으로 있으면, 두 구분자 사이에는 빈 문자열이 있다고 간주합니다.
```
